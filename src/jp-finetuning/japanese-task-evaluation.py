...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required="False",
        help="type of peft: lora (now adaptor of lora can be used)",
    )
    parser.add_argument(
        "--model_artifact_name",
        type=str,
        required="False",
        help="artifact path of adaptor",
    )
    parser.add_argument(
        "--hf_model_name",
        type=str,
...
if __name__ == "__main__":
    ...
    with wandb.init(project=arg.wandb_project, entity=arg.wandb_entity, config=arg, job_type="eval") as run:
        config = wandb.config
        table_contents = []
        ...
        if "Llama-2" in config.hf_model_name:
            ...
            temperature = 0
        template_type = config.prompt

        if use_local_model=="no":
            ...
            model_ar = wandb.use_artifact(config.model_artifact_name)
